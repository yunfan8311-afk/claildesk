...
def generate_control_file(version):
    ...
    content = """Package: claildesk
Version: %s
Architecture: %s
Maintainer: claildesk <info@claildesk.com>
Homepage: https://claildesk.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/lib/claildesk')
    system2('mkdir -p tmpdeb/etc/claildesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/claildesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/claildesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/lib/claildesk/')
    system2(
        'cp ../res/claildesk.service tmpdeb/usr/share/claildesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/claildesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/claildesk.svg')
    system2(
        'cp ../res/claildesk.desktop tmpdeb/usr/share/applications/claildesk.desktop')
    system2(
        'cp ../res/claildesk-link.desktop tmpdeb/usr/share/applications/claildesk-link.desktop')
    system2(
        'cp ../res/com.claildesk.claildesk.policy tmpdeb/usr/share/polkit-1/actions/')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/claildesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/claildesk/')
    system2(
        'cp ../res/pam.d/claildesk.debian tmpdeb/etc/pam.d/claildesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/claildesk/files/polkit && chmod a+x tmpdeb/usr/share/claildesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file('usr/share/claildesk/files/systemd/claildesk.service')
    system2('dpkg-deb -b tmpdeb claildesk.deb;')
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/claildesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/lib/claildesk/')
    system2(
        'cp ../res/claildesk.service tmpdeb/usr/share/claildesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/claildesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibclaildesk.dylib target/release/libclaildesk.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    '''
    system2(
        "create-dmg --volname \"claildesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon claildesk.app 200 190 --hide-extension claildesk.app claildesk.dmg ./build/macos/Build/Products/Release/claildesk.app")
    os.rename("claildesk.dmg", f"../claildesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libclaildesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libclaildesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/claildesk.exe')
    os.chdir('../..')
    if os.path.exists('./claildesk_portable.exe'):
        os.replace('./target/release/claildesk-portable-packer.exe',
                   './claildesk_portable.exe')
    else:
        os.rename('./target/release/claildesk-portable-packer.exe',
                  './claildesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/claildesk_portable.exe')
    os.rename('./claildesk_portable.exe', f'./claildesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/claildesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/claildesk.exe')
        system2('mv target/release/claildesk.exe target/release/claildesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\claildesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/claildesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/claildesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/claildesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/claildesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv claildesk-%s-0-x86_64.pkg.tar.zst claildesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./claildesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/claildesk-%s-0.x86_64.rpm ./claildesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall claildesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/claildesk-%s-0.x86_64.rpm ./claildesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall claildesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/claildesk*.deb ./flutter/claildesk.deb')
    ...
    '''.format(pa))
                system2(
                    'create-dmg "claildesk %s.dmg" "target/release/bundle/osx/claildesk.app"' % version)
                os.rename('claildesk %s.dmg' %
                          version, 'claildesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple claildesk-{1}.dmg
    ...
    '''.format(pa, version))
                ...
                # build deb package
                system2(
                    'mv target/release/bundle/deb/claildesk*.deb ./claildesk.deb')
                system2('dpkg-deb -R claildesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/claildesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/claildesk-link.desktop tmpdeb/usr/share/applications/claildesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/claildesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/claildesk/')
                os.system('mkdir -p tmpdeb/etc/X11/claildesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/claildesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/claildesk.debian tmpdeb/etc/pam.d/claildesk')
                system2('strip tmpdeb/usr/bin/claildesk')
                system2('mkdir -p tmpdeb/usr/lib/claildesk')
                system2('mv tmpdeb/usr/bin/claildesk tmpdeb/usr/lib/claildesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/lib/claildesk/')
                md5_file('usr/share/claildesk/files/systemd/claildesk.service')
                md5_file('etc/claildesk/startwm.sh')
                md5_file('etc/X11/claildesk/xorg.conf')
                md5_file('etc/pam.d/claildesk')
                md5_file('usr/lib/claildesk/libsciter-gtk.so')
                system2('dpkg-deb -b tmpdeb claildesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('claildesk.deb', 'claildesk-%s.deb' % version)
