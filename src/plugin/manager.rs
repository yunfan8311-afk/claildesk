...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "claildesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/claildesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "claildesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "claildesk wants to uninstall the plugin",
    )
}
