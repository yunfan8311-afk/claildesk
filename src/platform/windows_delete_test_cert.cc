// https://github.com/claildesk/claildesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteclaildeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes claildesk Test certificates and wrong key stores
//
...
BOOL DeleteclaildeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteclaildeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteclaildeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteclaildeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteclaildeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
